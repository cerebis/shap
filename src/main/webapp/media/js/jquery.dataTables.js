	...
	$.fn.dataTable = function( oInit )
	{
		...
		function _fnAjaxUpdate( oSettings )
		{
			if ( oSettings.bAjaxDataGet )
			{
				...
				{
					aoData.push( { "name": "sSearchGlobal", "value": oSettings.oPreviousSearch.sSearch } );
					aoData.push( { "name": "bRegexGlobal",  "value": oSettings.oPreviousSearch.bRegex } );
					for ( i=0 ; i<iColumns ; i++ )
					{
						aoData.push( { "name": "sSearch["+i+"]",     "value": oSettings.aoPreSearchCols[i].sSearch } );
						aoData.push( { "name": "bRegex["+i+"]",      "value": oSettings.aoPreSearchCols[i].bRegex } );
						aoData.push( { "name": "bSearchable["+i+"]", "value": oSettings.aoColumns[i].bSearchable } );
					}
				}
				...
				if ( oSettings.oFeatures.bSort !== false )
				{
					...
					for ( i=0 ; i<iFixed ; i++ )
					{
						aoData.push( { "name": "iSortCol["+i+"]",  "value": oSettings.aaSortingFixed[i][0] } );
						aoData.push( { "name": "sSortDir["+i+"]",  "value": oSettings.aaSortingFixed[i][1] } );
					}
					
					for ( i=0 ; i<iUser ; i++ )
					{
						aoData.push( { "name": "iSortCol["+(i+iFixed)+"]",  "value": oSettings.aaSorting[i][0] } );
						aoData.push( { "name": "sSortDir["+(i+iFixed)+"]",  "value": oSettings.aaSorting[i][1] } );
					}
					
					for ( i=0 ; i<iColumns ; i++ )
					{
						aoData.push( { "name": "bSortable["+i+"]",  "value": oSettings.aoColumns[i].bSortable } );
					}
				}
